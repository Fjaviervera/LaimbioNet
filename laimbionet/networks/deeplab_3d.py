...
class Network_DeepLab_v3_xception:
    ...
    def conv_init(self, x, filters):
        ...
        with tf.variable_scope('conv_init', reuse=self.reuse):
            ...
            x = tf.layers.conv3d(
                x, filters, [3, 3, 3], padding=self.padding, strides=(1, 1,1))
    ...
    def build_net(self):
        ...
        with tf.variable_scope(self.name, reuse=self.reuse):
            ...
            entry_flow_filters = [self.filters*4]

            x = self.xception_flow( x,entry_flow_filters, strides = [1,1,2])





            entry_flow_filters = [self.filters*8, self.filters*8*3]
            ...
            middle_flow_filters = [self.filters*8*3]
            for _ in range(self.middle_blocks):
                print('middle block')
                x = self.xception_flow( x,middle_flow_filters)

            input_decoder_2 = x

            final_flow_filters1 = [[self.filters*3*8, self.filters*4*8, self.filters*4*8]]

            x = self.xception_flow( x,final_flow_filters1, strides = [1,1,2])


            final_flow_filters2 = [[self.filters*4*8, self.filters*6*8, self.filters*8*8]]

            x = self.xception_flow(x, final_flow_filters2, strides = [1,1,1], do_residual=False)
            ...
            x = self.decoder_flow(input_decoder_1, input_decoder_2,x, 256)
